...
const string strMessageMagic = "AppleCoin Signed Message:\n";
...
static const int64_t MAX_APPLE_PROOF_OF_STAKE = 0.05 * COIN;	// 5% annual interest
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, unsigned int nBits, unsigned int nTime, int nHeight)
{
    int64_t nRewardCoinYear;

	nRewardCoinYear = MAX_APPLE_PROOF_OF_STAKE;

	if(nHeight < YEARLY_BLOCKCOUNT)
		nRewardCoinYear = 0.15 * COIN;	// 15%
	else if(nHeight < (2 * YEARLY_BLOCKCOUNT))
		nRewardCoinYear = 0.075 * COIN;	// 7.5%
	else if(nHeight < (3 * YEARLY_BLOCKCOUNT))
		nRewardCoinYear = 0.0365 * COIN;	// 3.65%
	else 
		nRewardCoinYear = 0.0001 * COIN;	// 0.01%
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
		...
		int64_t nCalculatedStakeReward = GetProofOfStakeReward(nCoinAge, nFees, pindex->nBits, nTime, pindex->nHeight);
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AppleCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        //const char* pszTimestamp = "Applecoin be HOT";	//"20 Feb 2014 Bitcoin ATMs come to USA";
		const char* pszTimestamp = "Applecoin be HOT,J Create genesis is J";

        CTransaction txNew;
        txNew.nTime = 1404870318;	//1393221600;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1404870318;	//1393221600;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 1597426 : 1908795;	//!fTestNet ? 164482 : 216178;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x7cad431cf25b1d92e1b4a5ec39e0e3697a2df527b93fc3ac6bdf5d103fbc4aaa"));	//uint256("0x12630d16a97f24b287c8c2594dda5fb98c9e6c70fc61d44191931ea2aa08dc90"));
        block.print();
