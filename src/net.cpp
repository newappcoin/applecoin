...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "AppleCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][3] = {
    {"Applecoin.org", "seed.applecoin.org"},
    {"Applecoin2.org", "seed2.applecoin.org"},
    {"Applecoin3.org", "222.77.179.170"},
	{"Applecoin_jc.org", "113.10.236.23"},
	{"Applecoin_ls.org", "114.215.197.193"},
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("applecoin-dnsseed");
...
void ThreadDumpAddress(void* parg)
{
    ...
    RenameThread("applecoin-adrdump");
...
void ThreadOpenConnections(void* parg)
{
    ...
    RenameThread("applecoin-opencon");
...
void ThreadMessageHandler(void* parg)
{
    ...
    RenameThread("applecoin-msghand");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. AppleCoin is probably already running."), addrBind.ToString().c_str());
        else
